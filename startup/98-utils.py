...
def move_motor_with_retries(motor, target_pos, max_retries=3):
    '''
    try to move the motor up to max_retries times.
    '''

    for attempt in range(1, max_retries+1):
        try:
            yield from bps.mv(motor,target_pos)
            print(f'Motor {motor.name} move succeded on attempt {attempt}.')
            return True
...
# class show_ps(QtAwareCallback):
#     def stop(self,doc):
#         uid = doc['run_start']
#         ps_new(uid=uid,det='monitor', suffix='_3', der=True, plot=True)
#         # do plotting

class show_ps(QtAwareCallback):
    def __init__(self, det, suffix, der, **kwargs):
    ...
    def stop(self,doc):
        uid = doc['run_start']
        ps_new(uid=uid, det=self.det, suffix=self.suffix, der=self.der, plot=True)
        # do plotting

## this is an example using qt callback to plot inisde a plan
def check_plotting(det=quadem):
    
    cb=show_ps(det='monitor', suffix='_3', der=True)
    yield from bpp.subs_wrapper(bp.rel_scan([det],sh,-1,1,5,per_step=shutter_flash_scan),cb)
    peak_cen = ps_new(det='monitor', suffix='_3', der=True, plot=False)
    print(peak_cen)
    return peak_cen


def ps_new(uid='-1',det='default',suffix='default',shift=.5,logplot='off', der = False, plot=False):
    '''
    YG Copied from CHX beamline@March 18, 2018
    function to determine statistic on line profile (assumes either peak or erf-profile)
    calling sequence: uid='-1',det='default',suffix='default',shift=.5)
